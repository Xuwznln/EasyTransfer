...
from easytransfer.server.middleware.auth import TokenAuthMiddleware
from easytransfer.server.routes.files import create_files_router
from easytransfer.server.routes.info import create_info_router
from easytransfer.server.routes.auth import create_auth_router
...
def _propagate_hot_changes(
    ...
    if "role_quotas" in changes:
        from easytransfer.server.auth.models import RoleQuota
...
def create_app(settings: Optional[ServerSettings] = None) -> FastAPI:
    ...
    async def startup_event():
        ...
        if settings.config_watch:
            print(
                f"[Server] Config watch enabled "
                f"(interval: {settings.config_watch_interval}s)"
            )
    ...
    if settings.user_system_enabled:
        from easytransfer.server.auth.models import RoleQuota
        from easytransfer.server.auth.routes import create_user_router
        ...
        if settings.oidc_client_id and settings.oidc_client_secret:
            callback_url = (
                ...
                or f"http://localhost:{settings.port}/api/users/callback"
            )
        ...
        app.include_router(
            create_user_router(UserDBProxy(), _oidc_provider, parsed_role_quotas)
        )
    ...
    async def reload_config(request: Request):
        ...
        """
        from easytransfer.server.config import HOT_RELOADABLE_FIELDS
        ...
        if changes:
            ...
            app.state.config_mtime = _get_config_mtime(settings)
            change_summary = {
                k: {"old": _safe_repr(old), "new": _safe_repr(new)}
                for k, (old, new) in changes.items()
            }
            print(f"[Config] Hot-reloaded {len(changes)} field(s): "
                  f"{', '.join(changes.keys())}")
        else:
            change_summary = {}
        ...
        try:
            ...
            if new_mtime and old_mtime and new_mtime != old_mtime:
                ...
                if changes:
                    _propagate_hot_changes(app, settings, changes)
                    print(
                        f"[Config] Auto-reloaded {len(changes)} field(s): "
                        f"{', '.join(changes.keys())}"
                    )
...
# For uvicorn command line: uvicorn easytransfer.server.main:app
app = create_app()
