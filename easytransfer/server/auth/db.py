...
class UserDB:
    ...
    async def list_users(self) -> list[UserTable]:
        async with self._session() as session:
            result = await session.execute(
                select(UserTable).order_by(UserTable.created_at.desc())
            )
    ...
    async def _get_user_by_sub(
        self, session: AsyncSession, oidc_sub: str
    ) -> Optional[UserTable]:
        result = await session.execute(
            select(UserTable).where(UserTable.oidc_sub == oidc_sub)
        )
    ...
    async def list_groups(self) -> list[GroupTable]:
        async with self._session() as session:
            result = await session.execute(
                select(GroupTable).order_by(GroupTable.name)
            )
            return list(result.scalars().all())

    async def update_group_quota(
        self, group_id: int, quota: RoleQuota
    ) -> Optional[GroupTable]:
        async with self._session() as session:
    ...
    async def create_session(
        self, user_id: int, ttl_hours: int = 24 * 7
    ) -> SessionTable:
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        async with self._session() as session:
            result = await session.execute(
                select(SessionTable).where(SessionTable.expires_at < now)
            )
    ...
    async def complete_pending_login(
        self, state: str, session_token: str
    ) -> None:
        async with self._session() as session:
    ...
    async def cleanup_pending_logins(self, max_age_seconds: int = 600) -> int:
        ...
        async with self._session() as session:
            result = await session.execute(
                select(PendingLoginTable).where(
                    PendingLoginTable.created_at < cutoff
                )
            )
            rows = result.scalars().all()
            count = len(rows)
    ...
    async def get_effective_quota(
        self, user: UserTable, role_quotas: dict[str, RoleQuota]
    ) -> RoleQuota:
        ...
        return RoleQuota(
            max_storage_size=most_permissive(
                [c.max_storage_size for c in candidates]
            ),
            max_upload_size=most_permissive(
                [c.max_upload_size for c in candidates]
            ),
            upload_speed_limit=most_permissive(
                [c.upload_speed_limit for c in candidates]
            ),
            download_speed_limit=most_permissive(
                [c.download_speed_limit for c in candidates]
            ),
...
def build_database_url(
    ...
    mysql_user: str = "root",
    mysql_password: str = "",
    mysql_database: str = "easytransfer",
) -> str:
    ...
    if backend == "mysql":
        ...
        return (
            f"mysql+aiomysql://{mysql_user}{password_part}"
            f"@{mysql_host}:{mysql_port}/{mysql_database}"
        )
    elif backend == "postgresql":
        password_part = f":{mysql_password}" if mysql_password else ""
        return (
            f"postgresql+asyncpg://{mysql_user}{password_part}"
            f"@{mysql_host}:{mysql_port}/{mysql_database}"
        )
    else:
