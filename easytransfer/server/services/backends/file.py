...
class FileStateBackend(StateBackend):
    ...
    Suitable for:
    - Default production deployments (single process)
    - Development with state preservation
    - Simple setups without external dependencies
    
    Limitations:
    - Not suitable for multi-worker deployments (no locking between processes)
    ...
    async def connect(self) -> None:
        ...
        if self.state_file.exists():
            try:
                ...
                print(f"[{self.name}] Loaded {len(self._data)} keys from {self.state_file}")
            except Exception as e:
                print(f"[{self.name}] Error loading state: {e}, starting fresh")
                self._data = {}
        else:
            print(f"[{self.name}] Initialized (file storage at {self.storage_path})")
    ...
    async def _save_loop(self) -> None:
        ...
        while True:
            try:
                ...
                print(f"[{self.name}] Error saving state: {e}")
    ...
    async def _save_state(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                print(f"[{self.name}] Error writing state file: {e}")
    ...
    async def scan(
        ...
        async with self._lock:
            ...
            all_keys = [
                ...
                if not self._is_expired(entry) and fnmatch.fnmatch(key, match)
            ]
