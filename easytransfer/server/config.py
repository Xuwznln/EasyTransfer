...
class ServerSettings(BaseSettings):
    ...
    storage_path: Path = Field(
        Path("./storage"), description="Path to store uploaded files"
    )
    chunk_size: int = Field(DEFAULT_CHUNK_SIZE, description="Default chunk size")
    max_upload_size: Optional[int] = Field(
        None, description="Maximum single upload size (None = unlimited)"
    )
    max_storage_size: Optional[int] = Field(
        ...
        description="Maximum total storage size in bytes (None = unlimited). "
                    ...
                    "Env: EASYTRANSFER_MAX_STORAGE_SIZE (supports suffixes: 100MB, 1GB, etc.)",
    )

    # State Backend - no longer requires Redis by default
    state_backend: Literal["memory", "file", "redis"] = Field(
        "file",
        description="State backend type: memory (test), file (default), redis (multi-worker)"  # noqa: E501
    )
    redis_url: str = Field(
        DEFAULT_REDIS_URL,
        description="Redis connection URL (only used if state_backend=redis)"
    )

    # Auth
    auth_enabled: bool = Field(True, description="Enable token authentication")
    auth_tokens: list[str] = Field(
        default_factory=list, description="Valid API tokens"
    )
    ...
    token_retention_policies: dict = Field(
        ...
        description="Per-token retention policy overrides. "
                    ...
                    "Env: EASYTRANSFER_TOKEN_RETENTION_POLICIES as JSON.",
    )
    ...
    default_retention_ttl: Optional[int] = Field(
        None,
        description="Global default TTL in seconds (for ttl retention). "
                    "Env: EASYTRANSFER_DEFAULT_RETENTION_TTL",
    )
    ...
    advertised_endpoints: list[str] = Field(
        ...
        description="Advertised IP addresses for clients (e.g., ['192.168.1.1', '10.0.0.1']). "
                    ...
                    "Env: EASYTRANSFER_ADVERTISED_ENDPOINTS as JSON array.",
    )
    ...
    user_system_enabled: bool = Field(
        ...
        description="Enable the user system (OIDC login, roles, groups, per-user quotas). "
                    "Env: EASYTRANSFER_USER_SYSTEM_ENABLED",
    )
    ...
    oidc_issuer_url: str = Field(
        ...
        description="OIDC issuer URL (e.g. https://auth.example.com). "
                    ...
                    "Env: EASYTRANSFER_OIDC_ISSUER_URL",
    )
    oidc_client_id: str = Field(
        "",
        description="OIDC client ID. Env: EASYTRANSFER_OIDC_CLIENT_ID",
    )
    oidc_client_secret: str = Field(
        "",
        description="OIDC client secret. Env: EASYTRANSFER_OIDC_CLIENT_SECRET",
    )
    oidc_callback_url: str = Field(
        "",
        description="OIDC callback URL. "
                    "Env: EASYTRANSFER_OIDC_CALLBACK_URL. "
                    "Example: http://localhost:8765/api/users/callback",
    )
    oidc_scope: str = Field(
        "openid profile email",
        description="OIDC scopes. Env: EASYTRANSFER_OIDC_SCOPE",
    )

    # Database backend for user system
    user_db_backend: str = Field(
        "sqlite",
        description="User database backend: sqlite (default) or mysql. "
                    "Env: EASYTRANSFER_USER_DB_BACKEND",
    )
    user_db_path: str = Field(
        "",
        description="Path to user SQLite database (when backend=sqlite). "
                    "Default: <storage_path>/users.db. "
                    "Env: EASYTRANSFER_USER_DB_PATH",
    )
    # MySQL connection settings (when user_db_backend=mysql)
    mysql_host: str = Field(
        "127.0.0.1", description="MySQL host. Env: EASYTRANSFER_MYSQL_HOST"
    )
    mysql_port: int = Field(
        3306, description="MySQL port. Env: EASYTRANSFER_MYSQL_PORT"
    )
    mysql_user: str = Field(
        "root", description="MySQL user. Env: EASYTRANSFER_MYSQL_USER"
    )
    mysql_password: str = Field(
        "", description="MySQL password. Env: EASYTRANSFER_MYSQL_PASSWORD"
    )
    mysql_database: str = Field(
        "easytransfer",
        description="MySQL database name. Env: EASYTRANSFER_MYSQL_DATABASE",
    )
    ...
    role_quotas: dict = Field(
        ...
        description="Per-role quota defaults. Env: EASYTRANSFER_ROLE_QUOTAS as JSON.",
    )
    ...
    cleanup_interval: int = Field(
        3600, description="Cleanup interval in seconds"
    )
    upload_expiration_hours: int = Field(
        24, description="Hours before incomplete uploads expire"
    )

    # Config file watching (hot-reload)
    config_watch: bool = Field(
        False,
        description="Watch config file for changes and auto-reload hot settings. "
                    "Env: EASYTRANSFER_CONFIG_WATCH",
    )
    config_watch_interval: int = Field(
        30,
        description="Config file watch interval in seconds. "
                    "Env: EASYTRANSFER_CONFIG_WATCH_INTERVAL",
    )

    class Config:
        env_prefix = "EASYTRANSFER_"
...
def _parse_yaml_to_settings_dict(config: dict) -> dict:
    ...
    if "storage" in config:
        ...
        if "max_storage_size" in st:
            ...
            d["max_storage_size"] = (
                parse_size(str(v)) if isinstance(v, str) else v
            )
...
def _apply_env_overrides(settings_dict: dict) -> None:
    ...
    env_endpoints = os.environ.get("EASYTRANSFER_ADVERTISED_ENDPOINTS", "")
    if env_endpoints:
        try:
            ...
            settings_dict["advertised_endpoints"] = [
                v.strip() for v in env_endpoints.split(",") if v.strip()
            ]

    env_max_storage = os.environ.get("EASYTRANSFER_MAX_STORAGE_SIZE", "")
    if env_max_storage:
...
_CONFIG_SEARCH_PATHS = [
    ...
    Path.home() / ".easytransfer" / "server.yaml",
]
...
def discover_config_path() -> Optional[Path]:
    ...
    Search order:
      1. ``$EASYTRANSFER_CONFIG`` environment variable
      2. ``./config.yaml``
      3. ``./config/config.yaml``
      4. ``~/.easytransfer/server.yaml``

    Returns:
        Path to the discovered config file, or None.
    """
    env_path = os.environ.get("EASYTRANSFER_CONFIG", "")
    if env_path:
        p = Path(env_path)
        if p.exists():
            return p
        print(f"[Config] WARNING: $EASYTRANSFER_CONFIG={env_path} does not exist")
...
def load_server_settings(config_path: Optional[Path] = None) -> ServerSettings:
    ...
    if resolved_path and resolved_path.exists():
        ...
        print(f"[Config] Loaded config from {resolved_path.resolve()}")
    else:
        print("[Config] No config file found, using defaults + environment variables")
