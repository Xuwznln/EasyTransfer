...
from easytransfer.common.constants import RedisKeys
from easytransfer.server.tus.models import TusUpload

if TYPE_CHECKING:
    from easytransfer.server.services.state import StateManager
    ...
    async def get_file_info(self, file_id: str) -> Optional[dict]:
        ...
        if upload:
            return {
                ...
                "retention_ttl": upload.retention_ttl,
                "retention_expires_at": (
                    upload.retention_expires_at.isoformat()
                    if upload.retention_expires_at else None
                ),
    ...
    async def get_storage_usage(self) -> dict:
        ...
        if self.max_storage_size:
            ...
            result["usage_percent"] = round(
                (used / self.max_storage_size) * 100, 2
            ) if self.max_storage_size > 0 else 0
