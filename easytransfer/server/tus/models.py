...
class TusMetadata(BaseModel):
    ...
    def from_header(cls, header_value: str) -> "TusMetadata":
        ...
        return cls(
            ...
            retention_ttl=(
                int(metadata["retention_ttl"])
                if metadata.get("retention_ttl")
                else None
            ),
        )
    ...
    def to_header(self) -> str:
        ...
        if self.retention_ttl is not None:
            items.append(
                f"retention_ttl {base64.b64encode(str(self.retention_ttl).encode()).decode()}"
            )
...
class TusCapabilities(BaseModel):
    ...
    checksum_algorithms: list[str] = Field(
        default_factory=lambda: ["sha1", "sha256", "md5"]
    )
