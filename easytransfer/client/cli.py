...
from easytransfer.common.constants import DEFAULT_CHUNK_SIZE, DEFAULT_SERVER_PORT
...
# ---------------------------------------------------------------------------
# Client config helpers  (~/.easytransfer/client.json)
# Single-server model: one server address + cached session token.
# ---------------------------------------------------------------------------

CLIENT_CONFIG_DIR = Path.home() / ".easytransfer"
...
def _get_server_url() -> str:
    ...
    if not url:
        print_error(
            "Server not configured. Run [bold]easytransfer setup <address>[/bold] first."
        )
        raise typer.Exit(1)
    return url
...
def setup(
    ...
    Examples:
        ...
        easytransfer setup http://myserver.example.com:8765
    """
    import httpx
    ...
    if auth_required and not cfg.get("token"):
        console.print()
        print_info("Run [bold]easytransfer login[/bold] to authenticate.")
    ...
    try:
        ...
        with create_transfer_progress() as progress:
            ...
            with EasyTransferClient(server, token=token, chunk_size=chunk_size) as client:
                ...
                location = uploader.upload(wait_on_quota=wait_on_quota)
...
def download(
    ...
    try:
        ...
        downloader = ChunkDownloader(server, token=token)
        ...
        if info.available_size < info.size:
            print_warning(
                ...
                f"available (upload in progress)"
            )
        ...
        elapsed = time.time() - start_time
        avg_speed = info.available_size / elapsed if elapsed > 0 else 0
...
def login(
    ...
    Run ``easytransfer setup <address>`` first to configure the server.
    """
    import httpx
    ...
    console.print(Panel(
        ...
        f"[link={authorize_url}]{authorize_url}[/link]",
    ...
    with console.status("[bold cyan]Waiting for login...", spinner="dots") as status:
        while elapsed < timeout:
            ...
            status.update(
                f"[bold cyan]Waiting for login... ({elapsed}s / {timeout}s)"
            )

    print_error(f"Login timed out after {timeout}s")
    raise typer.Exit(1)
...
def whoami(
    ...
    if not token:
        print_error("Not logged in. Run [bold]easytransfer login[/bold] first.")
        raise typer.Exit(1)
    ...
    try:
        ...
        if r.status_code == 401:
            print_error("Session expired. Run [bold]easytransfer login[/bold] again.")
            raise typer.Exit(1)
        r.raise_for_status()
        user = r.json()

        r2 = httpx.get(
            f"{server}/api/users/me/quota", headers=headers, timeout=10
        )
...
def logout():
    ...
    if token and server:
        try:
            headers = {"Authorization": f"Bearer {token}"}
            httpx.post(
                f"{server}/api/users/logout", headers=headers, timeout=10
            )
        except Exception:
            pass
    ...
    legacy_session = Path.home() / ".easytransfer" / "session"
...
def client_status():
    ...
    if not cfg.get("server"):
        print_warning(
            "Not configured. Run [bold]easytransfer setup <address>[/bold] first."
        )
        return
...
def gui():
    ...
    try:
        from easytransfer.client.gui import run_gui
        run_gui()
    except ImportError as e:
        print_error(f"GUI dependencies not installed: {e}")
        console.print("[dim]Install with: pip install easytransfer[gui][/dim]")
        raise typer.Exit(1)
    except Exception as e:
...
def server_start(
    ...
    If --config is not provided, the config file is auto-discovered from:
      1. $EASYTRANSFER_CONFIG env var
      2. ./config.yaml
      3. ./config/config.yaml
      4. ~/.easytransfer/server.yaml
    """
    from easytransfer.server.config import discover_config_path
    ...
    try:
        from easytransfer.server.main import run_server
...
def server_reload(
    ...
    if not token:
        print_error(
            "No admin token found. Ensure auth.tokens is set in config YAML, "
            "or provide --token."
        )
        raise typer.Exit(1)
    ...
    from easytransfer.common.constants import AUTH_HEADER
...
def server_config(
    ...
    The generated config is meant to be placed in the config/ folder:
      easytransfer server config -o config/config.yaml
...
def version():
    ...
    from easytransfer import __version__
