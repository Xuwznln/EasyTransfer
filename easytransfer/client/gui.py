"""Tkinter GUI for EasyTransfer client."""
...
class EasyTransferGUI:
    ...
    def _create_upload_tab(self, parent):
        ...
        self.upload_progress = ttk.Progressbar(
            progress_frame, variable=self.upload_progress_var, maximum=100
        )
        ...
        self.upload_btn = ttk.Button(
            parent, text="Upload", command=self._start_upload, state=DISABLED
        )
    ...
    def _create_download_tab(self, parent):
        ...
        self.download_progress = ttk.Progressbar(
            progress_frame, variable=self.download_progress_var, maximum=100
        )
    ...
    def _connect_server(self):
        ...
        def connect():
            try:
                from easytransfer.client.server_info import check_server_health
        ...
        def upload():
            try:
                ...
                def progress_callback(uploaded: int, total: int):
                    ...
                    self.msg_queue.put(
                        ("upload_status", f"Uploading: {format_size(uploaded)} / {format_size(total)}")
                    )
