...
async def test_download_once_consume_and_quota(tmp_path: Path):
    ...
    try:
        ...
        try:
            ...
            resp = await client.get(f"/api/files/{file_id}/download")
            assert resp.status_code == 200
...
async def test_quota_full_then_consume_then_resume(tmp_path: Path):
    ...
    try:
        ...
        try:
            ...
            # Upload 2: 8KB → should fail with 507 (4096 + 8192 > 8192)
            data2 = b"B" * 8192
            ...
            # PATCH should be rejected with 507
            patch_headers = _tus_headers(
                **{
                    "Upload-Offset": "0",
                    "Content-Type": "application/offset+octet-stream",
            ...
            resp = await client.get(f"/api/files/{file_id_1}/download")
            assert resp.status_code == 200
            ...
            # Retry upload 2 → should succeed now
            resp = await client.patch(f"/tus/{file_id_2}", headers=patch_headers, content=data2)
